...
public class StarModel implements Runnable{
    ...
    private Integer triangle1Tracker = 10;
    private Integer triangle2Tracker = 10;
    private Integer triangle3Tracker = 10;
    private Integer triangle4Tracker = 10;
    private Integer triangle5Tracker = 10;
    private Integer triangle6Tracker = 10;
    private boolean gameIsOver = false;
    private Piece winningPlayer = null;
    ...
    public Object clone() {
        try {
            return (StarModel) super.clone();
        } catch (CloneNotSupportedException e) {
            //return new Address(this.street, this.getCity(), this.getCountry());
            System.out.println("We could not clone");
    ...
    public List<MoveSnapshot> getAllPossibleMoves(SnapShot snap, Piece player) {
        List<MoveSnapshot> allPossibleMoves = new ArrayList<>();
        for(Marble marble: mapForMarbles.values()) {
            if(marble.getPlayer().getText() == player.getText()) {
                this.fromXY.x = marble.getMarbleX();
                this.fromXY.y = marble.getMarbleY();
                getAllMoves();

                if(listMap.size() > 0) {
                    for(Point point: listMap.keySet()) {
                        allPossibleMoves.add(new MoveSnapshot(getFromXY(), point));
                    }
                }
            }
        }
    ...
    public Stack<MoveSnapshot> getMoveHistory() {
        return moveHistory;
    }
