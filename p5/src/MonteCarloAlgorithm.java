 ...
 * using equals().
 * <p>
 * Note that no mechanism is provided to cancel running the algorithm once
 * it starts.  You might want to do this, for example, if the user hits "undo"
 * while the algorithm is calculating a move.  Since there is no mechanism for
 * this, it's best to just let the algorithm run to completion in this case,
 * and then throw away the (now obsolete) result.  As a pragmatic matter, one
 * second of calculation time has been found to generate good results, so the
 * delay to the user isn't large.
 */
public abstract class MonteCarloAlgorithm<GameStateType, MoveType, TeamType> {
    ...
    protected MonteCarloAlgorithm(double secondsPerTurn, int maxMovesPerPlay) {
        ...
        // 500 is a good value for Chinese checkers
    }
            ...
            Map<GameStateType, Visit<GameStateType>>  visits)
    {
        ...
        for (int i = 0; i <= maxMovesPerPlay; i++) {
            ...
            for (MoveType move : getLegalMoves(currState)) {
     ...
     * the state of the game with that move taken.
     */
